...
/** :: candidate -> string */
const getName = ({ id, email, firstName, lastName }) => isNull(firstName)
  ? email
  : `${firstName} ${lastName}`
...
const getLatestRequest = requests => requests
  .reduce((r1, r2) => moment(r1.created_at) > moment(r2.created_at) ? r1 : r2)
...
const getLatestOrg = ({ requests }) => requests.length > 0
  ? getLatestRequest(requests).orgName
  : '––'

/**::officer name */
const getLatestOfficerName = ({ checkerFirstName,checkerLastName,requests }) => requests.length > 0
  ? checkerFirstName+ " " +checkerLastName : '––'

/** :: candidate -> string */
const getLatestRequestDate = ({ requests }) => requests.length > 0
  ? moment(getLatestRequest(requests).created_at).format('YYYY-MM-DD')
  : '––'
      ...
      <Table className="CandidatesTable__table candidate_table_layout">
        {/* <thead>
          <tr>
            ...
            <TableHeading>Latest Role</TableHeading>
            <TableHeading>Latest Request</TableHeading>
            <TableHeading>Officer Name</TableHeading>
            <TableHeading>Status</TableHeading>
            <TableHeading>Last Nudge</TableHeading>
            <TableHeading></TableHeading>
          </tr>
        </thead> */}
        ...
        <thead>
          <tr className="candidate_row_vt candidate_row_heading">
            <th className="TableHeading candidate-image"></th>
            ...
            <th className="TableHeading candidate-org">
              <div className="candidate-heading-container">
                ...
                <span>Verifying Organisation</span>
              </div>
            </th>
              ...
              const assignStatus = candidate.status.value === 'No Request' ? false : candidate.assignStatus
                ...
                return (
                  <tr
                    ...
                    key={candidate.id}
                    onClick={() => (
                      history.push(`/organisations/${organisationId}/verifyingorganisations/${candidate.organisationId}/externalCandidates/${candidate.id}`)
                    )}
                  >
                    <TableData className="CandidatesTable__table-data" top={isFirst}>
                      <span className="CandidatesTable__avatar CandidatesTable__person">
                        <Avatar imgUrl={candidate.profileImage} alt={candidate.name} />
                      </span>
                    </TableData>
                    ...
                    {/* <TableData
                      className="CandidatesTable__table-data CandidatesTable__table-data--view cell-width"
                      top={isFirst}
                    >
                      View
                    </TableData> */}
